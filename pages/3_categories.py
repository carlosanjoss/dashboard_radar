...
from components.charts import CATEGORY_PALETTE, SEQUENTIAL_BLUE, SEQUENTIAL_MINT, SEQUENTIAL_WARM, base_plotly_layout
...
from services.queries import (
    ...
    get_pred_category_by_platform,
    get_pred_category_distribution,
    get_probability_by_category,
)


st.set_page_config(page_title="Categorias | Radar", layout="wide", initial_sidebar_state="expanded")
...
render_hero(
    "Categorias",
    'Tipologias <span class="hero-gradient">Gemma</span>',
    "Mapa das tipologias de discurso de ódio e das categorias preditas pelo modelo.",
)
...
metric_grid(
    [
        ("Tipos hate", format_int(type_df["hate_type"].nunique() if not type_df.empty else 0)),
        ("Menções", format_int(type_df["total_mentions"].sum() if not type_df.empty else 0)),
        ("Categorias Gemma", format_int(category_df["pred_category_item"].nunique() if not category_df.empty else 0)),
        ("Categoria líder", category_df.iloc[0]["pred_category_item"] if not category_df.empty else "sem dados"),
    ]
)
...
with left:
    section_header("Tipos de hate", "Frequência absoluta e percentual de cada hate_type.")
    if type_df.empty:
        st.info("Nenhum hate_type encontrado.")
    else:
        fig = px.bar(
            type_df,
            x="total_mentions",
...
with right:
    section_header("Categorias preditas", "Campo pred_category, separado quando há múltiplas categorias.")
...
section_header("Rede x tipo de hate", "Matriz de distribuição das tipologias por plataforma.")
...
with right:
    section_header("Probabilidade média", "Média de probabilidade por categoria predita.")
    prob_df = get_probability_by_category(filters, 20)
    if prob_df.empty:
        st.info("Sem probabilidade por categoria.")
    else:
        fig = px.scatter(
            prob_df,
            x="total",
        ...
        fig.update_layout(
            **base_plotly_layout(height=430, margin=dict(t=24, b=18, l=12, r=12)),
            xaxis_title="Ocorrências",
            yaxis_title="Probabilidade média de hate",
            legend_title="Categoria",
        )
        st.plotly_chart(fig, width="stretch")

section_header("Categorias por rede", "Tabela agregada para auditoria.")
category_platform_df = get_pred_category_by_platform(filters, 200)
st.dataframe(
    category_platform_df.rename(
        columns={
            "platform_label": "Rede",
            "pred_category_item": "Categoria",
            "total": "Ocorrências",
        }
    )[["Rede", "Categoria", "Ocorrências"]],
    width="stretch",
    height=440,
)
